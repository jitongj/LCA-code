...
%% plot figure 1b
% Load this data to save time from training the model (NOTE: this data is
% not public)
% load('data_training_small_range.mat') % bigdata small range, r=1
...
% 初始化热图矩阵
heatmap_data1 = nan(length(unique_y_labels), length(unique_x_labels));

% 填充热图矩阵
for i = 1:size(Perf1, 1)
    y_idx = find(unique_y_labels == y_labels(i));
    x_idx = find(unique_x_labels == x_labels(i));
    heatmap_data1(y_idx, x_idx) = values(i);
end
